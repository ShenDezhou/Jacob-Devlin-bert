...
df = pd.read_csv("../data/SC/train_5k.tsv", sep="\t")
gold_list = df["label"].tolist()

df_pred = pd.read_csv("5k_test_results.tsv", sep="\t", header=None)
pred_list = []
...
print(hard_df.info())
hard_df.to_csv("train_5k_hard.tsv", sep="\t", index=False) #1509
same_df = df.iloc[same]
print(same_df.info())
same_df.to_csv("train_5k_same.tsv", sep="\t", index=False) #3491